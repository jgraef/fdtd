    ...
    fn create_projection(
        ...
        let staging = {
            ...
            let bytes_per_row_padded =
                bytes_per_row_unpadded.max(wgpu::COPY_BYTES_PER_ROW_ALIGNMENT);
            let staging_size = bytes_per_row_padded as u64 * size.y as u64;
    ...
    Target: ImageTarget<Pixel = image::Rgba<u8>>,
{
    fn add_projection(&mut self, projection: &'a mut ImageProjection<Target>) {
        ...
        self.command_encoder.copy_texture_to_buffer(
            ...
            wgpu::TexelCopyBufferInfo {
                ...
                layout: wgpu::TexelCopyBufferLayout {
                    bytes_per_row: Some(projection.staging.bytes_per_row_padded),
                    ..Default::default()
                },
            },
            wgpu::Extent3d {
        ...
        self.copy_to_image_buffer.push({
            ...
            let bytes_per_row_padded = projection.staging.bytes_per_row_padded as usize;
...
impl<'a> FdtdWgpuProjectionPass<'a> {
    fn new(instance: &'a FdtdWgpuSolverInstance, state: &'a FdtdWgpuSolverState) -> Self {
        // we don't need the state here because the bind groups already reference the
        // buffers. all we could do here is that it's actually the correct state
        // object
        let _ = state;
