...
/// access to the underlying value. However, it only permits _immutable_, or
/// _shared_ access to the underlying value when that value is [`Sync`].
///
/// While this may seem not very useful, it allows `Exclusive` to
/// _unconditionally_ implement `Sync`. Indeed, the safety requirements of
/// `Sync` state that for `Exclusive` to be `Sync`, it must be sound to _share_
/// across threads, that is, it must be sound for `&Exclusive` to cross thread
/// boundaries. By design, a `&Exclusive<T>` for non-`Sync` T has no API
/// whatsoever, making it useless, thus harmless, thus memory safe.
///
/// Certain constructs like [`Future`]s can only be used with _exclusive_
/// access, and are often `Send` but not `Sync`, so `Exclusive` can be used as
/// hint to the Rust compiler that something is `Sync` in practice.
///
/// ## Examples
///
...
impl<T: ?Sized> Exclusive<T> {
    ...
    pub const fn from_mut(r: &'_ mut T) -> &'_ mut Exclusive<T> {
        // SAFETY: repr is ≥ C, so refs have the same layout; and `Exclusive` properties
        // are `&mut`-agnostic
